...
void InitializeD3Devices::InitD3D(HWND hwnd, uint32_t width, uint32_t height, uint32_t bpp)
{
	...
	if (r != S_OK)
	{
		CUBOID_CORE_ERROR("Could not create IDirect3DDevice10");
	}
...
void  InitializeD3Devices::CreateDepthStencilBuffer() 
{
	...
	if (FAILED(r))
	{
		CUBOID_CORE_ERROR("Unable to create depth buffer");
	}
	...
	if (FAILED(r))
	{
		CUBOID_CORE_ERROR("Could not create depth/stencil state");
	}
	...
	if (FAILED(r))
	{
		CUBOID_CORE_ERROR("Could not create depth/stencil view");
	}
}
